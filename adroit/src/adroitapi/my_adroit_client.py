...
class MyAdroitClient(AdroitClient):
    ...
    def generate_mandelbrot(
        ...
        resolution = Mandelbrot_pb2.IntegerOrderedPair(
            x=width,
            y=height
        )
        ...
        bounding_box = Mandelbrot_pb2.BoundingBox(
            corner1=Mandelbrot_pb2.ComplexNumber(
                real=str(corner1_real),
                imaginary=str(corner1_imaginary)
            ),
            corner2=Mandelbrot_pb2.ComplexNumber(
                real=str(corner2_real),
                imaginary=str(corner2_imaginary)
            )
        )

        bounds = Mandelbrot_pb2.BoundsDescriptor(
